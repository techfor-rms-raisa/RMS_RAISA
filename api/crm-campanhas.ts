 ...
 *  - v1.3 (08/06/2026 - Fase B): aceita `data_encerramento` (DATE) em
  ...
  try {
      ...
      if (action === 'criar_campanha') {
        ...
        const { data, error } = await supabase
          ...
          .insert({
            ...
            data_encerramento: dataEncerramentoFinal, // 🆕 v1.3 (Fase B)
            criado_por
          })
          .select()
          .single();
    ...
    if (req.method === 'PATCH') {
      ...
      if (action === 'atualizar_campanha') {
        ...
        // 🆕 v1.3 (Fase B): `data_encerramento` adicionado à lista permitida.
        ...
        if (updateData.data_encerramento !== undefined && updateData.data_encerramento !== null && updateData.data_encerramento !== '') {
