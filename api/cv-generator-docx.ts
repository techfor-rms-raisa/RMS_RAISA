...
// ============================================
// LOGO T-SYSTEMS (PNG extraído do template oficial)
// ============================================
...
// ============================================
// GERADOR DOCX T-SYSTEMS — baseado no template oficial
// Fonte: Century Schoolbook | Cor: #E20074 | Corpo: #595959
// ============================================
async function gerarDocxTSystems(dados: any): Promise<Buffer> {
  const FONT_TS = 'Century Schoolbook';
  ...
  // Helper: parágrafo título de seção (magenta bold)
  ...
  // Helper: parágrafo corpo (cinza)
  function tsBody(text: string, opts?: { bold?: boolean; italic?: boolean; size?: number; color?: string; before?: number; after?: number; justified?: boolean }): Paragraph {
    const o = opts || {};
    return new Paragraph({
  ...
  // Helper: célula de tabela T-Systems
  function tsCell(text: string, width: number, opts?: { header?: boolean; bold?: boolean; color?: string; bg?: string }): TableCell {
    const o = opts || {};
    ...
    return new TableCell({
      ...
      children: [new Paragraph({
        children: [new TextRun({
          text: text || '-', size: 18, font: FONT_TS,
          color: textColor, bold: o.bold || o.header
        })]
      })]
    });
  }

  // ─── CABEÇALHO: Logo + Caixa Magenta (Nome/Título/Cliente) ───
  // Logo T-Systems no header (alinhado à direita)
  let headerParagraph: Paragraph;
  try {
    const logoBuffer = Buffer.from(TSYSTEMS_LOGO_BASE64, 'base64');
    headerParagraph = new Paragraph({
      alignment: AlignmentType.RIGHT,
      children: [
        new ImageRun({
          data: logoBuffer,
          transformation: { width: 208, height: 46 }, // 5.5cm x 1.2cm em pixels
          type: 'png'
        })
      ]
    });
  ...
  const docHeader = new Header({ children: [headerParagraph] });

  // ─── BLOCO NOME/TÍTULO/CLIENTE (caixa magenta simulada com shading) ───
  // Linha magenta grossa como separador visual de topo
  children.push(new Paragraph({
    spacing: { before: 0, after: 0 },
    border: { bottom: { style: BorderStyle.SINGLE, size: 36, color: MAGENTA, space: 0 } },
    children: []
  }));

  // Nome em magenta grande
  children.push(new Paragraph({
    spacing: { before: 120, after: 40 },
    children: [new TextRun({ text: (dados.nome || '').toUpperCase(), bold: true, size: 50, font: FONT_TS, color: MAGENTA })]
  }));

  // Título da vaga
  children.push(new Paragraph({
    spacing: { before: 0, after: 40 },
    children: [new TextRun({ text: dados.titulo_vaga || dados.titulo_profissional || '', size: 24, font: FONT_TS, color: CINZA })]
  }));

  // Protocolo (código da vaga)
  if (dados.codigo_vaga) {
    children.push(new Paragraph({
      spacing: { before: 0, after: 40 },
      children: [new TextRun({ text: dados.codigo_vaga, size: 18, font: FONT_TS, color: CINZA })]
    }));
  }
  ...
  children.push(new Paragraph({
    ...
    children: [new TextRun({ text: dados.cliente_destino || 'T-Systems do Brasil', size: 18, font: FONT_TS, color: CINZA })]
  }));

  // ─── PERFIL ───
  if (dados.resumo) {
    children.push(tsTitle('PERFIL:'));
    const perfilParas = dados.resumo.split('\n').filter((p: string) => p.trim());
    perfilParas.forEach((p: string) => {
      children.push(tsBody(p.trim(), { justified: true, after: 60 }));
    });
  }

  // ─── HARD SKILLS (tabela 2 colunas) ───
  const hardSkills = dados.hard_skills_tabela || [];
  if (hardSkills.length > 0) {
    children.push(tsTitle('Hard Skills'));
  ...
  if (dados.parecer_entrevista_tecnica) {
    children.push(tsTitle('Parecer da Entrevista Técnica'));
    ...
    parecerParas.forEach((p: string) => {
      children.push(new Paragraph({
        ...
        children: [new TextRun({ text: p.trim(), size: 18, font: FONT_TS, color: CINZA })]
      }));
    });
    children.push(new Paragraph({ spacing: { after: 80 } }));
  }
  ...
  // ─── RECOMENDAÇÃO ───
  children.push(tsTitle('RECOMENDAÇÃO:'));
  children.push(new Paragraph({
    spacing: { after: 60 },
    shading: { fill: MAGENTA_CLARO, type: ShadingType.CLEAR },
    indent: { left: 100, right: 100 },
  ...
  const experiencias = dados.experiencias || [];
  if (experiencias.length > 0) {
    children.push(tsTitle('EXPERIÊNCIA PROFISSIONAL:'));
    ...
    experiencias.forEach((exp: any) => {
      ...
      // CONSULTORIA/CLIENTE em magenta + período à direita
      children.push(new Paragraph({
  ...
  // ─── IDIOMAS (texto simples, padrão do template) ───
  const idiomas = dados.idiomas || [];
  if (idiomas.length > 0) {
    children.push(tsTitle('Idiomas:'));
    idiomas.forEach((i: any) => {
      children.push(tsBody(`${i.idioma || ''} – ${fmtIdioma(i.nivel || '')}`, { color: CINZA }));
    });
  }

  // ─── FORMAÇÃO (texto simples, padrão do template) ───
  const formacoes = dados.formacao_academica || [];
  if (formacoes.length > 0) {
    children.push(tsTitle('FORMAÇÃO:'));
    formacoes.forEach((f: any) => {
      children.push(tsBody(
  ...
  const complementar = dados.formacao_complementar || [];
  if (complementar.length > 0) {
    children.push(tsTitle('Certificações/Cursos:'));
  ...
  // ─── INFORMAÇÕES ADICIONAIS ───
  children.push(tsTitle('INFORMAÇÕES ADICIONAIS:'));
  if (dados.disponibilidade) {
    children.push(tsBody(`Disponibilidade: ${dados.disponibilidade}`, { color: CINZA }));
  }
  if (dados.modalidade_trabalho) {
    ...
    children.push(tsBody(`Atuação: ${modalMap[dados.modalidade_trabalho] || dados.modalidade_trabalho}`, { color: CINZA }));
  }
  if (dados.cidade || dados.estado) {
    children.push(tsBody(`Cidade/Estado: ${[dados.cidade, dados.estado].filter(Boolean).join(' – ')}`, { color: CINZA }));
  }
  if (dados.nivel_hierarquico) {
    ...
    children.push(tsBody(`Nível Hierárquico: ${nivelMap[dados.nivel_hierarquico] || dados.nivel_hierarquico}`, { color: CINZA }));
  }
