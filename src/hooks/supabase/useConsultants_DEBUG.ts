...
export const useConsultants = () => {
  ...
  const batchAddConsultants = async (newConsultants: Omit<Consultant, 'id'>[]) => {
    try {
      ...
      const { data, error } = await supabase
        .from('consultants')
        .insert(newConsultants.map((c, idx) => {
          let pessoa = null;
          if (c.cpf) pessoa = cvMap.get(`cpf:${c.cpf}`);
          if (!pessoa && c.email_consultor) pessoa = cvMap.get(`email:${c.email_consultor}`);
          
          // 🔍 DEBUG: Mostrar dados recebidos (primeiro registro)
          if (idx === 0) {
            console.log('🔍 DEBUG useConsultants - Dados recebidos do CSV:');
            console.log('  c.celular:', c.celular);
            console.log('  c.valor_pagamento:', c.valor_pagamento);
            console.log('  (c as any).dt_aniversario:', (c as any).dt_aniversario);
            console.log('  (c as any).cnpj_consultor:', (c as any).cnpj_consultor);
            console.log('  (c as any).empresa_consultor:', (c as any).empresa_consultor);
            console.log('  Objeto c completo:', JSON.stringify(c, null, 2));
          }
          ...
          // 🔍 DEBUG: Mostrar dados que serão inseridos (primeiro registro)
          if (idx === 0) {
            console.log('🔍 DEBUG useConsultants - Dados a inserir no Supabase:');
            console.log('  celular:', insertData.celular);
            console.log('  valor_pagamento:', insertData.valor_pagamento);
            console.log('  dt_aniversario:', insertData.dt_aniversario);
            console.log('  cnpj_consultor:', insertData.cnpj_consultor);
            console.log('  empresa_consultor:', insertData.empresa_consultor);
          }
