  ...
  const carregarVagasAbertas = useCallback(async (): Promise<VagaResumo[]> => {
    ...
    try {
      ...
      const vagas: VagaResumo[] = (data || []).map((v: any) => ({
        ...
        stack_tecnologica: Array.isArray(v.stack_tecnologica) ? v.stack_tecnologica : (v.stack_tecnologica ? String(v.stack_tecnologica).replace(/[\[\]{}"]/g, '').split(',').map((s: string) => s.trim()).filter(Boolean) : []),
        status: v.status,
