...
export async function generatePredictiveAlert(recentFlags: BehavioralFlag[]): Promise<string> {
    ...
    try {
        ...
        // response é o próprio result
        return result.text || "Análise preditiva inconclusiva.";
    } catch (error) {
...
export async function analyzeReport(reportText: string, managerName?: string): Promise<AIAnalysisResult[]> {
  ...
  try {
    ...
    const jsonString = result.text.replace(/^```json/i, '').replace(/^```/i, '').replace(/```$/i, '').trim();
...
// Importar tipo do interviewService para compatibilidade
