 ...
 *
 * Versão: 1.1-trace (diagnóstico de deploy)
 * Data: 12/03/2026
 */

import React, { useState, useCallback, useEffect } from 'react';

const TALENT_FINDER_VERSION = '1.1-trace';
...
function exportarXLS(candidatos: CandidatoEncontrado[]) {
    ...
    const rows = candidatos.map(c => [
        c.nome_completo, c.cargo_atual, c.empresa_atual || '',
        c.cidade || '', c.estado || '', c.linkedin_url || '',
...
const TalentFinderTab: React.FC = () => {
    ...
    // ── TRACE: confirma que o componente carregou ──────────────────
    useEffect(() => {
        console.log(`✅ [TalentFinderTab v${TALENT_FINDER_VERSION}] Montado com sucesso`);
        console.log(`📡 [TalentFinderTab] Endpoint: /api/talent-finder-search`);
    }, []);
    ...
    const handleBuscar = useCallback(async () => {
        ...
        console.log(`🚀 [TalentFinderTab v${TALENT_FINDER_VERSION}] Iniciando busca`);
        console.log(`   Requisitos: ${requisitos.substring(0, 100)}`);
        console.log(`   Max: ${maxResultados}`);
        ...
        try {
            const resp = await fetch('/api/talent-finder-search', {
                ...
                body: JSON.stringify({ requisitos: requisitos.trim(), max_resultados: maxResultados }),
            });

            console.log(`📥 [TalentFinderTab] HTTP ${resp.status}`);
            const data = await resp.json();
            console.log(`📦 [TalentFinderTab] Resposta:`, data);

            if (!resp.ok || !data.success) throw new Error(data.error || 'Erro ao buscar candidatos.');

            const lista: CandidatoEncontrado[] = (data.resultados || []).map((c: any) => ({ ...c, selecionado: false }));
            console.log(`✅ [TalentFinderTab] ${lista.length} candidatos recebidos`);
    ...
    return (
        <div className="space-y-5">
            ...
            {/* ── Badge de versão (diagnóstico — remover após validar) ── */}
            <div className="flex justify-end">
                <span className="text-xs bg-yellow-100 text-yellow-700 border border-yellow-300 px-2 py-0.5 rounded-full font-mono">
                    🔧 TalentFinder v{TALENT_FINDER_VERSION}
                </span>
            </div>
            ...
            <div className="bg-white border rounded-xl p-5 space-y-4 shadow-sm">
                <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">
                        Requisitos Mandatórios <span className="text-red-500">*</span>
                    </label>
                    <textarea
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 pt-2 border-t">
                    <div className="flex items-center gap-3 flex-1">
                        <label className="text-sm text-gray-600 whitespace-nowrap">
                            Máx. resultados: <span className="font-bold text-indigo-700 ml-1">{maxResultados}</span>
                        </label>
                        <input type="range" min={5} max={50} step={5} value={maxResultados}
                            onChange={e => setMax(Number(e.target.value))} disabled={searchState.loading}
                ...
                <div className="bg-white border rounded-xl p-8 text-center shadow-sm">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1">Executando até 6 buscas no Google. Isso pode levar 20–40 segundos.</p>
                        </div>
                        <span className="animate-pulse text-xs text-gray-400">● Buscando perfis públicos</span>
                    </div>
                </div>
            )}
                ...
                <div className="bg-red-50 border border-red-200 rounded-xl p-5">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <button onClick={handleBuscar} className="mt-3 text-sm text-red-700 underline hover:no-underline">
                                Tentar novamente
                            </button>
                        </div>
                    </div>
                ...
                <div className="space-y-4">
                    <div className="bg-white border rounded-xl px-4 py-3 shadow-sm flex flex-wrap items-center justify-between gap-3">
                        <div className="flex items-center gap-4">
                            <span className="font-semibold text-gray-700">
                                {resultados.length === 0 ? '0 candidatos encontrados'
                        ...
                        {resultados.length > 0 && (
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={handleExportar}
                                    ...
                                    {selecionados.length > 0 ? `Exportar ${selecionados.length} selecionados` : 'Exportar todos (.csv)'}
                                </button>
                            </div>
                        )}
                    </div>
                    ...
                    {resultados.length === 0 && (
                        <div className="bg-white border rounded-xl p-10 text-center shadow-sm">
                            ...
                            <p className="text-sm text-gray-400 mt-2">Tente reformular os requisitos com mais palavras-chave técnicas.</p>
                        </div>
                    )}
                    ...
                    {resultados.length > 0 && (
                        <div className="space-y-2">
                            {resultados.map((c, idx) => {
                                ...
                                return (
                                    <div key={c.finder_id}
                                        className={`bg-white border rounded-xl p-4 shadow-sm transition-all ${c.selecionado ? 'border-indigo-300 bg-indigo-50/30' : 'hover:border-gray-300'}`}>
                                        <div className="flex items-start gap-3">
                                            <input type="checkbox" checked={!!c.selecionado}
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex flex-wrap items-center gap-2">
                                                    <span className="font-semibold text-gray-800 text-sm">{c.nome_completo}</span>
                                                    <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${rel.color}`}>{rel.label}</span>
                                                    {c.linkedin_url && (
                                                        <span className="text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 border border-blue-200 font-medium">LinkedIn</span>
                                                    )}
                                                </div>
                                                <p className="text-sm text-gray-600 mt-0.5">
                                                    {c.cargo_atual}
                                                    {c.empresa_atual && <span className="text-gray-400"> · {c.empresa_atual}</span>}
                                                </p>
                                                {(c.cidade || c.estado) && (
                                                    <p className="text-xs text-gray-400 mt-0.5">📍 {[c.cidade, c.estado].filter(Boolean).join(', ')}</p>
                                                )}
                                            ...
                                            <div className="flex-shrink-0 flex flex-col gap-1.5 items-end">
                                                {c.linkedin_url ? (
                                                    ...
                                                    // Sem URL direta — busca nome + empresa para máxima precisão
                                                    // "Luis Trindade CI&T" identifica o perfil com alta confiança
                                                    // Só nome seria impreciso; nome + empresa resolve na maioria dos casos
                                                    <a href={`https://www.linkedin.com/search/results/people/?keywords=${encodeURIComponent(
                                                            c.empresa_atual
                                                                ? `${c.nome_completo} ${c.empresa_atual}`
                                                                : c.nome_completo
                                                        )}`}
                                                        ...
                                                        title={c.empresa_atual
                                                            ? `Buscar "${c.nome_completo}" na "${c.empresa_atual}" no LinkedIn`
                                                            : `Buscar "${c.nome_completo}" no LinkedIn`}>
                                                        <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                                                            <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                                                        </svg>
                                                        {c.empresa_atual ? 'Buscar no LI' : 'Buscar no LI'}
                                                    </a>
                                                )}
                                            </div>
                                        </div>
                        ...
                        <details className="bg-gray-50 border rounded-lg">
                            ...
                            <div className="px-4 pb-3 space-y-1">
                                {queriesGoogle.map((q, i) => (
                                    <p key={i} className="text-xs text-gray-500 font-mono bg-white rounded px-2 py-1 border">{i + 1}. {q}</p>
                                ))}
                            </div>
                        </details>
                    )}
