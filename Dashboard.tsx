...
import { Consultant, Client, User, UsuarioCliente, CoordenadorCliente, ConsultantReport, RiskScore } from '../src/components/types';
    ...
    return relevantClients.map(client => {
        ...
        const managers = clientManagers.map(manager => {
            ...
            if (!isQuarantineView) managerConsultants = managerConsultants.filter(c => c.ano_vigencia === selectedYear);
                ...
                const isRisk = c.parecer_final_consultor === 4 || c.parecer_final_consultor === 5;
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {manager.consultants.map(consultant => (
                                        <tr key={consultant.id}>
                                            ...
                                            <td className="px-4 py-2 text-center"><StatusCircle score={consultant.parecer_final_consultor} isFinal={true} /></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
