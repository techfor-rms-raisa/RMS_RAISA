/**
 * AnaliseRisco.tsx - RMS RAISA v2.0
 * Componente de Análise de Risco de Candidatos
 * 
 * ATUALIZADO: 28/12/2024
 * - Integrado com predicaoRiscosService
 * - Adicionado painel de alertas proativos
 * - Adicionado métricas do modelo
 */

import React, { useState } from 'react';
import { analyzeCandidate } from '../../services/raisaService';
import { RiskFactor } from '@/types';
import PredicaoRiscosPanel from './PredicaoRiscosPanel';

const AnaliseRisco: React.FC = () => {
    const [resumeText, setResumeText] = useState('');
    const [isAnalyzing, setIsAnalyzing] = useState(false);
    const [risks, setRisks] = useState<RiskFactor[] | null>(null);
    const [abaAtiva, setAbaAtiva] = useState<'analise' | 'alertas' | 'metricas'>('analise');
            ...
            {abaAtiva === 'analise' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
                    <div className="bg-white p-6 rounded-lg shadow-md flex flex-col">
                        ...
                        <textarea 
                            className="flex-1 w-full border rounded-lg p-4 font-mono text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none bg-gray-50 min-h-[300px]"
